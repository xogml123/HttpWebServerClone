...
Response::Response(Request *req)
{
	...
	if (_location && !_location->getCgiExtension().empty())
		_cgi = new Cgi(req);
	else
		_cgi = NULL;
...
void    Response::setStatus(int code)
{
	...
	if (this->_location && !this->_location->getReturn().second.empty())
	{
		if (is_valid_path(ROOT_PATH + this->_location->getReturn().second))
			code = this->_location->getReturn().first;
		else
			code = 404;
	}
...
void    Response::setHeader(void)
{
	...
	else if (this->_status.first >= 300 && this->_status.first <= 400)
	{
		...
		this->_header += this->_location->getReturn().second + NEW_LINE;
	}
}
...
void	Response::setErrorContent(void)
{
	...
	it = this->_request->getServer()->getErrorPages().find(this->_status.first);
	// Default error page setup case 
	if (it != this->_request->getServer()->getErrorPages().end() && is_valid_path(it->second))
	{
...
bool	Response::isMethodAllowed(const std::string &method)
{
	...
	if (this->_location)
	{
		for (size_t i = 0; i < this->_location->getMethods().size(); ++i)
		{
			if (method == this->_location->getMethods()[i])
				return true;
		}
	}
	this->setStatus(405);
...
void	Response::methodGet(const std::string &file_content)
{
	...
	/* autoindex case */
	if (isDir(this->_request->getConstructPath()))
	{
		if (this->_location && this->_location->getAutoIndex() == AUTOINDEX_ON)
...
void	Response::methodPost(void)
{
	...
	bool isUpload = false;
	if (this->_location && !this->_location->getUploadStore().empty())
	{
		isUpload = uploadFile();
		if (this->_status.first != 200)
			return ;
	...
	else if ((size_t)this->_request->getServer()->getClientMaxBodySize() != 0 &&
		this->_request->getContent().size() > (size_t)this->_request->getServer()->getClientMaxBodySize())
		this->setStatus(413);
	else
...
bool	Response::uploadFile(void)
{
	...
	const std::string	absolutePath = ROOT_PATH + _location->getUploadStore();
	...
	for (info_type::iterator it = fileInfo.begin(); it != fileInfo.end(); ++it)
	{
		...
		if (_request->getServer()->getClientMaxBodySize() &&
			getFileLength(toUploadPath) > _request->getServer()->getClientMaxBodySize())
		{
			setStatus(413);
			return false;
		}
